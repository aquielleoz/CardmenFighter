...
async function waitLog(p,re){ for(let i=0;i<50;i++){ if((await log(p)).some(l=>re.test(l))) return true; await wait(150); } return false; }
(async()=>{
  ...
  await host.goto(url('host')); await join.goto(url('join')); await wait(1100);
  ...
  ok(hp.replace(/^You played/,'')===jp.replace(/^Rival played/,''),'both sides describe the same card ('+hp+' / '+jp+')');

  // the CLIENT plays: mirrored phrasing
  // the client's mirror is ROTATED so its own seat is index 0 — its turn is turn===0, not 1
  for(let i=0;i<60 && (await turnOf(join))!==0;i++) await wait(150);
  ok((await turnOf(join))===0,'it is now the client\'s turn (rotated seat 0)');
