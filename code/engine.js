/* ============================================================================
 * CARDMEN FIGHTER — Engine core (v0.1, pure logic, no DOM)
 * ----------------------------------------------------------------------------
 * The Chikicha combat engine rebuilt for the Cardmen Fighter TCG:
 *   - 40-card deck per player: values 1-10 in four suits.
 *   - Value ranking: 1 (low) .. 10 (high). NO supreme card, NO four-of-a-kind bomb.
 *   - The card's number is its fight value AND its energy cost (they are one and the same).
 *   - Combos: single, pair, trio, straight, full house. NO flush and NO straight
 *     flush — a run of the same suit is just a straight (same rule for every deck).
 *   - Straights: 1-2-3-4-5 (lowest) .. 6-7-8-9-10 (highest); no wrap.
 *   - Duel (1v1): 4 shields each. Round = draw, one player leads, the fight is a
 *     Chikicha trick; the last unbeaten play wins the round. Winning with a COMBO
 *     strips 1 opponent shield; winning with a single just banks energy. At 0
 *     shields, the next combo win is the FIGHTER KICK -> that player loses.
 *   - Every card you fight goes to your Energy pile (spending it comes with the
 *     effects layer — not in this core).
 *
 * This core has NO card effects yet (the 0/STOPPER, draws, ramp, equipment come
 * next). It is deliberately UI-free.
 * ========================================================================== */
(function (root) {
  'use strict';
  ...
  function sortHand(hand) {
    return hand.slice().sort(function (a, b) {
      var va = fightValue(a), vb = fightValue(b);   // sort by ladder value (= rank when REWORK off) so Ace/2 sort up top
  ...
  function detectStraight(cards) {
    ...
    // Ordered by FIGHT VALUE (= rank when REWORK is off). Values are contiguous 3..15 under REWORK
  ...
  function detectCombo(cards) {
    ...
    var n = cards.length, ranks = cards.map(fightValue);   // "ranks" here = fight VALUES (= rank when REWORK off)
  ...
  function enumerateCombos(hand) {
    var out = [], byRank = {};                                                      // keyed by FIGHT VALUE (= rank when REWORK off)
  ...
  function costReq(card) {
    ...
    var r = card ? activationCost(card) : 0;                                      // activation cost (= card number when REWORK off; J/Q/K = 10, A = 1)
  ...
  function buildDeck(key) {
    ...
    return out;                                     // 40 cards (off) / 52 cards (REWORK)
  }
  ...
  function resolveEffect(st, p, card, eff, opts) {
    ...
    switch (eff.kind) {
      ...
      case 'shield':
        ...
        var stargets = eff.shieldAll ? st.players : [pl];   // Sanctuary (REWORK): shieldAll → every player gains, so it's a wash on the shield race (the nerf)
        ...
        spendCard(pl, card); break;
      case 'ward':                                         // Leyline base (REWORK): pure can't-lose, no ramp/recycle
      ...
      case 'lockout':                                     // Back Stab: the target skips their NEXT turn (no fights, no Techniques)
        ...
        if (eff.lockRound) st.players[oppIdx].lockRound = true;   // Perseus / Hermes (REWORK): skip the WHOLE round
        spendCard(pl, card); break;
      case 'equip':
        pl.equipment.push({
      ...
      case 'recycle':
        ...
        else if (eff.scope === 'opp') { recyc(oppIdx); }   // Pandora (REWORK): Poison hits only the Rival
        else recyc(p);
        spendCard(pl, card); break;
