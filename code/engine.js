...
(function (root) {
   ...
   *                   The alternative — expressing "unbeatable" as a RULE inside beats() rather than as a value
   *                   — was prototyped in 2026-08 and dropped; the Infinity value is what ships. Nothing should
   *                   depend on that representation, though: `apexOnlyPlay` accepts 15 and Infinity both, which
   *                   is what lets the chop answer an unbeatable 2 either way.
  ...
  var CLEANUP_EVENTS = {
    ...
    expire: function (st) { st.players.forEach(function (pl) { pl.preventShield = false; pl.nextPlayBoost = 0; pl.shieldImmune = false; pl.cantLoseRound = false; pl.finishingBlow = false; pl.lockRound = false; }); }, // GUARD, pre-fight boost, immunity, can't-lose, Finishing Blow, and a whole-round lock all last only their round
    ...
    stampRound: function (st, result) { result.newRound = st.round; }
  };
  var CLEANUP_ORDER = ['roundAdvance', 'initiative', 'pileClear', 'expire', 'equipReset', 'temps', 'stampRound'];
