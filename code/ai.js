...
(function (root) {
  ...
  function lockoutReason(st, p, t, all) {
     ...
     *                        jab      11 casts -> 100.0%
     *                        NOTHING  51 casts ->   7.8%   (baseline is 50%)
     * and that last group was 48% of every duel cast. Any legal play is enough — it need not be a Special. */
    if (!E.legalFightPlays(st, p).length) return 'no-follow-up';
    ...
    return (E.aliveCount(st) <= LOCKOUT_MAX_ALIVE) ? 'crowd-thin' : 'crowded';   // see LOCKOUT_MAX_ALIVE — default never holds
  }

  /* "A high special defends itself, don't bother locking" was the biggest DISCRETIONARY hold in the model —
   * 15% of every evaluation at six players — and measured 2026-08-25 it bought nothing. Casting in those spots
   * instead DOUBLES the cast rate (25.6 -> 50.3 per 100 six-player games) with round-win over the same
   * chance-rounds unchanged (24.1% -> 25.4%, 0.9 s.e.) and deck spread unmoved (largest 2.0 s.e., 8 runs per
   * arm). 6 = never holds at any legal table size; the knob stays for future study. */
  var LOCKOUT_MAX_ALIVE = 6;
