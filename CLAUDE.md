...
npm test               # = node test.js && node netview.test.js — 403 + 60 assertions, must end 0 FAIL
...
node test.js                                    # engine + AI suite — 403 assertions, must end 0 FAIL
...
`test` 403, `netview` 60, `mptest` 85, `rulestest` 150, `landscapetest` 192, `decktest` 42, `viewtest` 21,
...
The 51 netplay suites: `nettest_3p` 7, `priosig` 8, `parkbeat3` 10, `stale` 7, `endscreen` 51, `lobbyback_rtc` 26, `remotetrim` 9, `desync` 7, `starter` 10, `mirrordrop` 10, `activate` 14, `actloop` 22, `ceremony` 9, `clientwin` 10, `concede3` 8,
