# Cardmen Fighter — changelog

**Newest first.** Split out of `NEXT-SESSION.md` on 2026-09-07, which had grown to 6,061 lines of which almost
...
first was 94% history. It is 293 lines now: build/test header, START HERE, and the ranked backlog.

Nothing was rewritten in the move. Two guards came with it: `versiontest` asserts **this** file carries a
`### vX.Y.Z` heading for the version in `README.md`, and — new — that **no version heading creeps back into
`NEXT-SESSION.md`**, since drifting into one file again is how it got to 6,061 lines the first time.

The split also demonstrated the rule it was done under (Aj, the same day: *"if we have to update multiple places
just to record, then one of those places will be forgotten and go stale"*): the changelog's **location** was
restated in **seven** places — CLAUDE.md ×5, README, PATCHNOTES — every one of which had to move with it.
