...
Current version: **v1.28.0**. The 2-apex + Forms **rework is simply the game** — the `REWORK` flag and the classic pre-rework rules were deleted in v1.23.0 (no `setRework`, no `E.isRework()`). Live MP rules: `chosen`/`targeted` toggles, set in the template.
...
- **Deck builder — "parts" system** — ✅ **BUILT** in v1.27.0 (solo/local) + v1.28.0 (netplay). See those changelog entries. Follow-up still open:
  - **Advanced tutorial lesson on custom decks** — nothing teaches the builder yet; a short lesson (parts → 52 cards → doubling a class duplicates its cards) would fit the Advanced track beside the energy-sorting lesson planned below.
