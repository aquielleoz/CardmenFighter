...
`resolutionGuardCard`, `resolutionResult`, `drainResolution`, `resolutionPushCard`, the `'resolution'` prompt-timing
...
The `'resolution'` id is **persisted in `localStorage`**, so it needs a read-time migration; `sel.mode` and
