...
**Test gate:** `npm test` = `node test.js` (**393**) + `node netview.test.js` (**47**). Both must end **0 FAIL**;
...
**Current version: v1.31.114.** The 2-apex + Forms **rework is simply the game** — the `REWORK` flag and the
...
`main` is at **v1.31.114**, working tree clean. The only branch is **`feat/qr-scanning`** (parked; its BACKLOG
