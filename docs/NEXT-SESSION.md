...
Current version: **v1.26.4**. The 2-apex + Forms **rework is simply the game** — the `REWORK` flag and the classic pre-rework rules were deleted in v1.23.0 (no `setRework`, no `E.isRework()`). Live MP rules: `chosen`/`targeted` toggles, set in the template.
...
Two notes for the next session: `browsertest.js` reporting "you: 0 | rival: 12" is **normal** — that harness drives the human seat with a dumb script (first legal card, else pass, never activates an effect), so it is a crash smoke test, not a balance signal. And **run the Playwright suites one at a time**: `nettest_full` timed out at >180s when the browser smoke test was running alongside the sweep, and passed 5/0 immediately when run alone.
