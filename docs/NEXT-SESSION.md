...
Build: `node build.js` inlines engine.js + ai.js + art.js + faces.js + **netview.js** → **CardmenFighter.html** (self-contained).
Test: `node test.js` (PASS / 0 FAIL, currently 684) · `node netview.test.js` (28, snapshot redaction) · `node browsertest.js` (headless duel smoke) · the `nettest_*.js` Playwright suite (full-UI netplay — `nettest_full`/`_counter`/`_activate`/`_discard`/`_guard`/`_ceremony`/`_deckpick` 2p, `_3p`/`_rtc`/`_rtc3`/`_react3`/`_target3`/`_losspick3`/`_losspick_remote3`/`_concede3`/`_prefight` N-player) · `node analysis.js 130 on x rework` (rework balance round-robin — args `N catchup recycle rework`; filler like `x` in the recycle slot to reach the rework flag) · `node mpsim.js` (3/4/6p free-for-all balance).
...
Current version: **v1.0-rework** (rework is the LIVE game — `E.setRework(true)` default-on. Live MP rules: `chosen`/`targeted` toggles, set in the template.)
...
- **Deck builder — "parts" system** (Aj): build a custom deck by choosing how many **parts** of each pure class to include. A **part = one complete suit, ranks 1–K (all 13 cards, A/2 included)** of that class. Parts stack, incl. multiples of the same class → duplicate cards. Example: *1 part Wizard + 2 parts Cleric + 1 part Fighter* = 13♦ + 26♥ + 13♣ = 52. The current presets are special cases (Pure = 4 parts of one class; Full Set = 1 part of each of the 4). **Players can name the decks they create** (saved/persisted). Open questions below — resolve before building.
  - RESOLVED (Aj): **always 4 parts** (52 cards, fixed). **No caps** — duplicating a class is allowed (4 parts one class = today's Pure).
  - RESOLVED (UI): keep the existing deck dropdown; add a labeled **"✏️ Custom deck…"** entry at the bottom (not a bare `...`). It opens a small builder — four steppers (Wizard/Cleric/Fighter/Rogue) that must sum to 4 parts, a live "= 52 cards" preview, and a **name** field. Saved decks then appear as their own named options in that same dropdown everywhere a deck is picked (incl. each MP seat). Presets = fixed distributions (Pure = 4/0/0/0, Sage = 2/2/0/0, Full Set = 1/1/1/1) and stay as-is.
  - RESOLVED (netplay): a deck is transmitted as its **part-composition** (e.g. `{D:1,H:2,C:1}`), NOT a saved name/key the host must recognize. The host builds the deck from the parts (shuffles with its own RNG); the **name is cosmetic display only** (unknown name → show the client's label or "Custom"). Generalize today's archetype-key channel to carry a composition. `buildDeck` currently takes a single suit key + `perSuit`; add a composition builder (parts → concat full 13-card suits).
  - Persistence: localStorage; saved decks list shared by the solo picker and MP seat pickers.
